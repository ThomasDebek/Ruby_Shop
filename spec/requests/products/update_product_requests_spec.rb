...
RSpec.describe 'PATH /administrator/products/:id', type: :request do
  ...
  context 'when logged in as administrator' do
    ...
    context 'with valid parameters' do
      before do
        patch administrator_product_url(product), params: { product:
    ...
    context 'with empty product\'s price' do
      it 'does not update a product ' do
        patch administrator_product_url(product), params: { product: { price: '' } }
        product.reload
    ...
    context 'with empty product\s name' do
      it 'does not update a product' do
        patch administrator_product_url(product), params: { product: { name: '' } }
        product.reload
